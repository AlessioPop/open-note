...
| `icons.js` | 66 | the line-icon set — `icn('table')` — and `defineIcon()` for a feature's own. Loads ahead of the items so they can register icons while loading |
| `glass.js` | 69 | the glass material and the warp that every floating panel shares |
| `palette.js` | 209 | the palette — shelves, tiles and search, drawn from the registry |
...
`cat` names a shelf — `write`, `math`, `media`, `shapes` or `decor`, declared in `ui/palette.js`; `defineToolCat()` adds another if a new area of the app ever deserves one. `icon` names a drawing in `ui/icons.js`, and `defineIcon()` registers one the set doesn't have. `order` sorts within the shelf; ties keep load order.
