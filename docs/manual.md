...
A **Feynman diagram** is the particle-physics companion to the molecule. Add it from **Science**, select it, and the same quiet glass rail appears at its left edge. The first chip opens a compact **Standard Model** instead of the periodic table: all six quarks, six leptons, the photon, gluon, W and Z fields, and the Higgs, with charge, spin and mass along its foot. Pick a field and drag its propagator between vertices. Fermions are straight with a charge-flow arrow, photons and electroweak bosons wave, gluons curl, and the Higgs is dashed.
...
- **The ghost is the promise.** Rest the pointer or drag and the exact propagator that would land is shown dashed in the accent colour. A forbidden interaction turns red before the pointer lifts, names the reason under the diagram, and is never committed.
- **A branch is one real vertex.** Start or finish a drag on the middle of a propagator and it splits at the snap point; the new leg and both halves now share the same interaction vertex. A tap on empty space starts a line upward, and a tap on a vertex offers the next leg into the clearest upward-facing gap. Three-leg interactions therefore settle naturally into a Y rather than three carefully aligned parallel strokes.
- **The validator knows the renormalizable Standard Model vertices:** electromagnetic and neutral-current fermion pairs; charged-current lepton and CKM quark pairs; quark–gluon vertices; Yukawa couplings; triple and quartic gluon vertices; charged electroweak gauge self-couplings; and the Higgs couplings to W, Z and itself. Fermion and charged-W arrows must flow through their vertices. An unfinished vertex that can still become one of these remains editable, but cannot be exported until it is complete.
...
- **The lasso follows the molecule logic.** Click a vertex to take its connected diagram, or draw a loop round only the vertices you want. The menu above the selection offers turn, move, copy and delete; turn snaps in 15° steps and every motion follows the pointer directly.
...
- **⟲ Tidy** lays the graph out in connected levels with time progressing upward, without changing particles, arrows, bends or momenta. **✎** controls the axes, chooses external-only, all, or hidden particle labels, and can hide interaction dots.
