...
Markdown is always stored as the plain text you wrote. The strip above the paper writes headings, `**bold**`, `*italic*`, inline code, fenced code, bullets, tasks, `---`, inline equations and display equations for you. `Ctrl`/`Cmd`+`B` and `Ctrl`/`Cmd`+`I` toggle bold and italic around the selection; `Ctrl`/`Cmd`+`E` opens Preview. Enter continues `- ` bullets, `- [ ]` tasks, numbered lists and quotes, and an empty marker ends the list. Tab and Shift+Tab move list lines in and out.
...
| Tool | What it does |
|---|---|
| Heading / Text / Handwriting | Poster type, serif body text, marker-pen handwriting — `$$…$$` compiles to a typeset equation, backticks to code, and `#` at the head of a line to a heading. The box is only as wide as its writing; see **How wide a writing box is** below |
...
| Pie chart / Bar chart / Stacked bar | Charts of named shares, typed straight into their own legend — a pie in four looks (flat, donut, 3D, hand-sketched), up to ten slices in six colour palettes, labels you can flip between four placements or simply drag where you want them; see **Charts** below |
...
| World | A map of the world — pan it, throw it and it glides, zoom about the pointer with the wheel or a pinch, and the capitals and then the cities come up to meet you as you go in. Tap a country to name it, search for one and the map walks there, press and hold one to pull it out onto the page. Land height, rivers and lakes are layers; see **The world** below |
| Country | One country on its own — its shape, its name and its capital, framed on itself, and its height, lakes and rivers if you want them, clipped to its own border. Drag one out of a world map, or pick it from the shelf and choose which |
...
The rest of the marks markdown taught everyone, and they work in every box that takes writing, alongside equations and backticks. Like those, they compile when you leave the box and come back as what you typed when you double-click in again — the hashes and the dashes are what is stored, so a heading or a list survives the backup, the export and the print as the lines that made it.

- **`# ` a heading, `## ` a sub-heading, `### ` the step under that.** The hashes must open the line and want a space after them — a `#` in the middle of a sentence is a hash, and `#1` is still a number. Four or more hashes are read as the third step rather than as nothing at all.
- **A heading keeps the face of the box it is in.** It is a heading of *your* writing: bigger and heavier — but handwriting stays handwriting and the serif stays serif, and the three steps differ only in size. Nothing is set in capitals it was not typed in.
- **`---` alone on a line draws a rule** across the writing, fading out at both ends. Three dashes or more; anything else on the line and they are only dashes.
- **A heading is still a line of writing**, so it can hold an equation or a phrase of code: `# The law of $$x^2$$` sets the formula in the heading. The other way round, a `#` inside a ```fence``` or inside a formula is left exactly where it is.
- **`- ` opens a bullet.** Press `Enter` and the next one is already there; press it on a bullet you have left empty and the list ends — the marker comes off and you are writing prose again.
- **`Tab` pushes a bullet one step in, `Shift`+`Tab` takes it back out**, up to six steps. A nested bullet is drawn hollow rather than solid, and **a hairline runs down from the bullet it belongs to**, so a run of them reads as one branch rather than as writing that happens to start further along. The indent is a tab in the writing itself, so two spaces do the same thing — and it is written straight into the box, since an editor handed a tab is apt to give back a space.
- **`- [ ] ` opens a task with a box to tick**, and `- [x] ` one already done. **Click the box** — no need to edit the line — and the task strikes itself through; what changes is the one bracket in the writing, so ticking something off is a change like any other and is in the next backup. `Enter` after a task makes another task, and after a *done* one makes an empty one: the next thing to do, not another thing done.
- **`**two stars**` are bold and `*one*` is italic**, and three of them are both at once. The stars want writing against them, so `2 * 3 * 4` stays arithmetic, and a star inside a `fence`, inside backticks or inside a formula is that language's own — `a**b` is a power in half the languages there are. Emphasis lives inside one line: a pair that would have to reach onto the next is only a pair of stars.
- **`Ctrl`+`B` and `Ctrl`+`I` do the same thing from the keyboard.** Pick out a word and they put the stars round it — press again and they take them off. With nothing picked out they write the pair and park the caret between them, the way `$` does. It is the *stars* they write, never hidden formatting: what you get from the shortcut is what you would have got by typing it, and the line still reads as itself in a backup or an export.
- **`->` is an arrow.** Two keystrokes anywhere in a sentence, and it comes out as →, drawn in the ink around it rather than set in whatever glyph the face happens to have. Inside a `fence`, inside backticks or inside a formula it stays as the two characters you typed, since that is what code and TeX mean by them.
...
For numbers you want to *show* rather than plot — what the month went on, what the build is made of. **Pie chart**, **Bar chart** and **Stacked bar** sit on the palette's Math shelf, all of them the same thing underneath: rows of a name and a number. **The legend under the picture is the editor** — click a name or a number and type, `Enter` on the last number starts the next row, ✕ takes one off — and the picture follows every keystroke. The bar and stacked charts are deliberately plain first versions; the family will grow.
...
- **The ghost.** Rest the pointer anywhere on the drawing and the next click is drawn before you make it — the atom, the bond, the whole ring, in the accent colour and dashed. It is the real drawing, not a hint: whatever the ghost shows is exactly what the click keeps.
- **Click empty paper** for an atom — or, within a bond's reach of an atom already there, a bond out of that atom towards you, **snapped** to one bond long and to the nearest 30°, so a chain never wanders off the angles a chemist would use. **Drag from an atom** and the same bond follows the pointer; **let go on another atom** and it bonds to that one instead. **Click an atom** of the element you are holding and a new bond grows into the widest gap — tapping the end of a chain lengthens it. Click it holding a different element and it is relabelled.
- **Click a bond** to step it single → double → triple, over any step the two ends could not hold: a C–O bond goes single, double, single, because oxygen has no third bond to give. `1` `2` `3` on the keys set it outright under the mouse, and the rail's bond button also offers a **wedge and a hash** for stereochemistry, and a **hydrogen bond**.
- **The hydrogen bond** is the sixth setting of the bond button, and it is drawn the way the book prints it: dots, D–H···A. **Drag from one atom to another** to lay one down, click it with the same pen to take it away, and the eraser takes it too. It can run between two molecules or fold back inside one — salicylic acid's own OH to its own carbonyl is a drag.
  A hydrogen bond is **not a bond**, and Open Note keeps that promise properly: it holds no electron pair, fills no valence, closes no ring, changes no hydrogen count, and cannot be written in SMILES — so it is kept in a list of its own that the chemistry never sees. Two waters joined by one are still **two molecules**, and the line under the drawing still says `H₄O₂ · water + water`. The drawing is the only thing that changes.
  It will not let you draw a nonsense one. One end must have a **lone pair to accept it** and one end must **carry a hydrogen to give**, or the dotted line goes red and the strip says which is missing — two carbons have nothing to offer each other, and neither do two carbonyl oxygens. C–H···O is allowed, because that one is real.
- **The chain** draws a zigzag. Drag it out of an atom, or off bare paper, and a **carbon chain** follows the pointer on the same 30° lattice every bond uses — with **a number over every carbon** as it grows, and the count under the drawing, so you can drag until it says the ten you wanted. It bonds onto anything it meets on the way.
- **Rings**: benzene first, then 6, 5, 3, 4, 7 and 8 — the ring button steps through them. Click empty paper for a free ring, **a bond to fuse** a ring onto it (naphthalene is two clicks), or **an atom to hang one** from it. **Drag instead of clicking** and you get a **row of fused rings** running after the pointer, each one numbered as it lands — anthracene is one gesture.
- **Shorten** a long chain. Click a run of plain CH₂ carbons and it folds into the **repeat bracket** — the run drawn once between square brackets with the count as a subscript, which is how a chemist writes ⟮CH₂⟯₁₆ rather than drawing sixteen of them. The drawing really does shrink; click the bracket to open it out again. Nothing leaves the molecule: the formula, the mass, the name, the SMILES and the 3D view are all exactly what they were, because the carbons are still there — only drawn short. Rings are never folded, and neither is anything carrying a charge, an isotope or a double bond: the bracket means *n of this*, and there is no honest way to say that about a fused ring.
- **Charge** puts + or − on the atom you click (click the rail button again to flip it; `+` and `-` on the keys do the same under the mouse). The **eraser** takes atoms and bonds away, and so does `Delete` under the mouse.
- **The lasso** picks things up. **Click any atom and the whole molecule under it is picked** — the useful thing when the sheet has half a reaction scheme on it — or **draw a loop** round part of one to take just that. A small menu appears over what you picked with **turn**, **chirality**, **move** and **copy** on it. *Copy* lays another one down beside it and hands you the copy. *Turn* and *move* arm the next drag rather than doing anything themselves, so you drag where you like instead of hunting for a handle: turn goes about the selection's own middle in 15° steps, with the angle shown under the drawing. Dragging an atom you have already picked moves the selection without arming anything, and `Esc` lets go.
- **Chirality** (⇄) inverts every selected stereocentre by exchanging its wedge and hash depth directions. It does **not** reflect or rotate the drawing, so the result is the same whichever way the molecule happens to face on the page. The line under the drawing says how many stereocentres changed; applying it twice restores the starting handedness. **Tidy preserves that stereo state** while rearranging the 2D coordinates, so a cleanup cannot silently undo the inversion.
- **Only molecules that could exist.** A click that would leave an atom holding more bonds than it can — a fifth on a carbon, a ring hung off a benzene carbon that already has three — is refused: the ghost turns red before you commit to it, the pointer says no, the line under the drawing names the atom and what it cannot hold, and the drawing shakes its head. Nothing is lost, because nothing was done. An atom that arrives over-full some other way — read in from a SMILES string — still wears the **red halo** it always did, and the pen is not locked because of it.
...
**⌕ asks for one by name.** Type `aspirin`, `glucose`, `caffeine`, `3-oxetanone` or the systematic alias `oxetan-3-one` — or a SMILES string like `CC(=O)O` — and it is drawn and laid out. Exact names use a direct lookup and suggestions begin with a binary search of the sorted alias index, so the large offline catalog does not turn typing into a scan through every structure. **⟲** tidies anything you drew by hand into the same layout. Six-membered rings are built on true regular-polygon arcs even when another ring constrains both ends, rather than the alternating long and short sides an approximate curve could make. Large branched structures are unfolded by turning whole rigid branches around their acyclic bonds until atoms no longer pile up and bonds no longer cross; ring polygons and bond lengths stay intact while the structure spreads out. Hydrogen-bonded pieces are arranged as a single dotted network, with each real molecule kept rigid, so tidy no longer pulls the covalent parts apart and then leaves crossed D–H···A lines between them. **◐** steps the style: skeletal → **condensed** (every carbon written, `CH₃`, `OH`) → **Lewis** (every hydrogen drawn and the lone pairs as dots). **ƒ** hides and shows the formula line. **Pull the corner dot** to resize the whole widget continuously — the 2D figure, 3D companion, labels and controls scale together. **⇩** exports either the current **2D drawing** or the current **3D pose** as crisp **SVG** or high-resolution **PNG**, always on a **transparent background** with flat element colours and black connections for a scientific paper. Its fifth choice copies only the 2D molecule's `\chemfig{…}` code to the clipboard, assuming ChemFig is already loaded; drawn angles, bond lengths, ring hooks, stereobonds and dotted hydrogen bonds are carried into the code, while internal double bonds use ChemFig's shifted skeletal form. **✎** turns the heteroatom colours off for a plain-ink drawing, and **Copy as SMILES** puts the line notation on the clipboard — paste it into the ⌕ box of any other molecule.

**3D.** A quiet **3D disclosure on the right edge** shows **>** while closed. Click it and the second live window opens fully beside the drawing while the mark becomes **<**; click **<** and it closes completely. The editable 2D structure stays in place, so you can draw and watch the molecule update in space at the same time. Its hydrogens are materialised and its geometry worked out: water bent at 104.5°, methane a tetrahedron, cyclohexane a chair, ethane staggered, a benzene ring flat, your wedges and hashes honoured. **Drag the companion to turn it, and let go with speed and it keeps turning**, dying away on its own; a fresh grab takes it back at once. The wheel sizes it, a double-click brings it home, and **◐** cycles **ball-and-stick**, **sticks** and **space-filling** while the companion is open.

The **highlighter under that pull-tab** is additive: pick as many atoms, ordinary bonds or hydrogen bonds as you need, and click any marked object again to remove just that one. Every mark appears in 2D and wears a translucent **aura in 3D**; the aura is painted in screen space above every sphere and stick, so even highlights on the far side stay visible while you turn the molecule. Clicking empty space or pressing `Esc` clears the set. Measurement picks in the 3D companion use a separate colour, so the line and distance made by two clicked atoms cannot be confused with a highlight.
...
A **Feynman diagram** is the particle-physics companion to the molecule. Add it from **Science**, select it, and the same quiet glass rail appears at its left edge. The first chip opens a compact **Standard Model** instead of the periodic table: all six quarks, six leptons, the photon, gluon, W and Z fields, and the Higgs, plus proton and neutron shortcuts in a clearly separated composite row, with charge, spin and mass along its foot. Pick a field or composite state and drag its propagator between vertices. Fermions are straight with a charge-flow arrow, photons and electroweak bosons wave, gluons curl, and the Higgs is dashed.

The small axes at the lower left define this editor's convention: **time runs upward** and the representative space direction runs right. They are orientation guides, not a claim that a drawn line is a measured classical trajectory. They are shown in the live widget, SVG, PNG and copied TikZ-Feynman; **✎** can hide them for a publication that states the convention elsewhere.

- **The ghost is the promise.** Rest the pointer or drag and the exact propagator that would land is shown dashed in the accent colour. Free propagators keep one consistent length and lock when the pointer comes close to 0°, 45°, 90°, 135° or 180° (and the same directions on the other half of the circle); the small angle mark appears while the lock is engaged. An existing vertex within reach still wins, so joining the graph remains effortless. A forbidden interaction turns red before the pointer lifts, names the reason under the diagram, and is never committed.
- **Parallel quark lines take one click each.** Hover just to either side of a propagator and the selected particle appears in a free lane on that same side; crossing the line moves the ghost across with the pointer. A centred pointer chooses the nearest free lane automatically. At a 45° corner both straight legs keep the standard propagator length and lattice angle, then meet at one crisp point; that internal join never acquires an extra particle label. Click to place exactly that ghost, then repeat for the three quark lines of a proton or neutron. **Alt-click** keeps the older replace-in-place action when the particle type of an existing propagator needs changing.
- **A branch is one real vertex.** Start or finish a drag on the middle of a propagator and it splits at the snap point; the new leg and both halves now share the same interaction vertex. A tap on empty space starts a 45° line upward, and a tap on a vertex offers the next leg into the clearest upward-facing gap. Three-leg interactions therefore settle naturally into a Y rather than three carefully aligned parallel strokes.
- **The validator knows the renormalizable Standard Model vertices** and the effective nucleon shortcuts: electromagnetic and neutral-current fermion pairs; charged-current lepton, CKM quark and proton–neutron pairs; quark–gluon vertices; Yukawa couplings; triple and quartic gluon vertices; charged electroweak gauge self-couplings; and the Higgs couplings to W, Z and itself. Fermion and charged-W arrows must flow through their vertices. An unfinished vertex that can still become one of these remains editable, but cannot be exported until it is complete.
- **Loop** is one gesture rather than a fight with two Bézier handles. Click a vertex for a self-loop, or drag between two vertices for a pair of opposite curved propagators with their arrows already circulating consistently. It is the quick route to vacuum-polarisation and one-loop topologies.
- **Momentum** (`k`) opens a small anchored field on the propagator. Type `k`, `p_1`, or LaTeX, flip which side it sits on, and the same `momentum=` or `momentum'=` key is carried into TikZ-Feynman.
- **The lasso follows the molecule logic.** Click a vertex to take its connected diagram, or draw a loop round only the vertices you want. The menu above the selection offers turn, move, copy and delete; turn snaps in 45° steps and every motion follows the pointer directly.
- **⌕ Common processes** is the quick start when the particle physics is rusty. Search names, reaction notation or topics such as `beta`, `QED`, `QCD`, `muon` or `Higgs`. The library contains beta-minus and beta-plus decay at quark level, muon decay, electron–positron annihilation, Compton scattering, pair production, quark gluon emission, gluon splitting, Higgs decay to W bosons and the three-gluon vertex. Each result is already a single connected, validated, upward-time graph; choosing one replaces the current diagram.
- **⟲ Tidy** lays the graph out on the same 45° lattice with the same standard propagator length and time progressing upward, without changing particles, arrows, bends or momenta. Pull the bottom-right corner dot to scale the entire diagram continuously. **✎** controls the axes, chooses external-only, all, or hidden particle labels, and can hide interaction dots.

**⇩ is three exports.** SVG and 2× PNG are transparent scientific artwork. **Copy LaTeX** writes no file: it puts only a pasteable `tikzpicture` / `feynman` block on the clipboard. The code uses explicit `\vertex ... at (...)` placement and `\diagram*`, so the upward-time arrangement on the page survives in Overleaf; its `\draw` commands carry the space and time axes, while propagator styles, curved loops, particle labels, edge labels, momentum side and arrow direction remain editable. Load `\usepackage[compat=1.1.0]{tikz-feynman}` in the document preamble, then paste the copied block where the figure belongs.

The **Periodic table** follows on the shelf: the table on the page as a reference card, the categories in muted tints, and a tap on any element writes its number, mass, electronegativity and electron configuration along its foot.

**Chart of nuclides** is a different chart altogether — the one that hangs on the wall of every nuclear physics department. Not the elements but the *nuclides*: every atomic nucleus that has ever been made or found, laid out with **neutrons across and protons up**, so that an element is a single row of it and the periodic table is what is left when you throw the horizontal axis away. There are 3558 of them, plus 2088 metastable states, and the numbers are NUBASE2020 — the evaluated table the wall charts are printed from.
...
The seven are Natural Earth's own, with two calls this app makes for the sake of the picture: **Russia is drawn with Asia**, where three quarters of its land is, because a Europe that reached Kamchatka would be a plate of Siberia with Europe down one edge of it; and Turkey, Cyprus and the Caucasus go with Asia the way the data has them. Nobody is wrong about either — a picture has to pick one.
...
**A continent comes off the same way**, and lands as the same card: `▣` on, press and hold anywhere in it — or simply drag it, once it is the picked one — and let go on the paper. **Continent** on the Science shelf makes one from scratch, starting at Africa.

A country card's own toolbar: `⌕` which country, `A` its name, `★` its capital, `◌` the countries around it drawn faintly for context, `▲` the height of its land, `◉` its lakes, `≈` its rivers, `◈` round or straight outlines, `◎` the projection, `⊗` unstick it from its neighbours. A card is framed on the *main body* of a country rather than on every island it owns — asking to be shown France means France, not the mid-Atlantic between Guiana and Réunion.

**A card is the country, not a card of it.** There is no paper behind it and no box round it — what sits on the sheet is the shape itself, with its own shadow following its own coastline. Give it a caption and the caption appears under it; leave it empty and there is nothing there to see.

### A plate of a continent

**A continent is the same card over a set of countries instead of one**, and it is drawn the way an atlas draws a plate: the whole landmass filled in one piece, the borders inside it as hairlines, the coast round the outside of it in the heavier pen, and the continent's name across the middle. `★` marks **the capitals in it** — as many as will fit, biggest city first, and a name that would land on another name or on the continent's own is simply not set, which is how a plate of Africa carries a dozen names rather than fifty-six. `▲`, `◉` and `≈` clip the height, the lakes and the rivers to the continent exactly as they clip to a country.

The union is not a drawing trick: the outlines share their borders, so a piece of outline that only *one* country of the set drew is the continent's coast and a piece two of them drew is a border between them. Both fall out of one pass, which is why the coast of Africa is the coast of Africa and not fifty-seven outlines stacked on each other.

A continent is built out of the **main body** of each of its countries, the same part a card of that country is framed on — so a plate of Europe is Europe, and not Europe drawn across the Atlantic to hold French Guiana.

**And a country clicks onto its continent.** Bring a card of France up against a card of Europe and it clicks into place *inside* it, at Europe's own scale, exactly where the world has France — because a continent's frame is the world's frame, the same one every other card is in.

### Countries that click together

**Drag two neighbours together and they click.** Bring a card of Spain up until it actually touches a card of France and France lights up; let go and Spain snaps into place with a *pop* — resized to France's own scale and set down exactly where the world has it. Not near it: *in register*, so the Pyrenees on one card are the Pyrenees on the other. Build outwards and you have Europe on your sheet, one country at a time, each of them still its own card with its own switches.

**Touching is the whole of what you have to do, and it is the whole of what is asked.** Nothing lights up until the two *countries* meet — not the cards, which carry a margin all round, and not a magnet reaching halfway across the sheet — so a card moved past another is only a card being moved. And two countries the world does not have touching will not click however you put them: Portugal will not go straight onto France, because Spain is between them, and a Japan dropped square on top of a Brazil is still a world away from it.

Bringing them together is not the same as putting one where it will end up, and it is not meant to be: two cards of the same width are drawn at two different scales, so a Spain that has to grow by half to sit under France could never be both. You bring them together; the click puts them right.

The pair is one thing from then on: **drag either of them and the whole arrangement comes**, and **resize either of them and the whole arrangement resizes**, because an arrangement is drawn at one scale or it is not an arrangement. Changing the projection of one changes it for all of them, for the same reason.

If you bring a big country up to a much smaller one — France to Luxembourg — it is the small one that moves, because Luxembourg's scale would draw France six sheets wide. The gesture is still yours either way: one card moves and it is the one that can.

**Two cards dropped on each other never make a folder.** Everything else on the page files itself away that way, and a card that has to be pushed up against another to click would do it by accident every time. A country still goes *into* a folder that is already there — drop it on one and in it goes, wearing its own shape as its icon — but a folder is never started with one.

**`⊗` pulls a card back out of the arrangement**, with the *pop* played backwards. Nothing moves when it does — the card is left exactly where you were looking at it — and the next drag takes it away on its own. A drag never breaks an arrangement by accident; only `⊗` does.

**Height, lakes and rivers are the map's own layers on the card, and they stop at the border.** That is the difference between the two pictures and it is the point of one: a map is a piece of the world and its rivers run off the edge of it, but a card is *one country*, and its water and its terrain belong to it. Egypt gets the Nile and Lake Nasser and nothing of Libya; Kenya gets its share of Turkana cut off exactly where the border cuts it. A country dragged off a map arrives wearing whatever the map was wearing, so a card pulled off a map showing height and rivers is a card showing height and rivers.
...
**And the steps are stepped, not stroked.** The zoom is a spring: it passes a level of detail on the way out, comes back over it and settles on it. Read the level straight off the zoom and each of those crossings is another rebuild, and with the height field on a rebuild is real arithmetic — which is what a zoom used to flicker with. So the level and the window come off the zoom in **whole octaves**, and it takes a good push past a boundary to leave the one you are on. Two and a half octaves of zoom is four rebuilds, not four hundred. On top of that the height field alone waits: while your hand is on the map, or the springs are still running, you get the bands already drawn, scaled — and the field is contoured once, when the map stands still, fading in over the ones it replaces. What you see going in is one smooth magnification and then detail arriving, which is what going in is supposed to look like.
...
A page full of loose shortcuts is still a mess. **Drag one thing on top of another and they file themselves into a folder** — the one underneath lights up while you hold the icon over it, and when you let go a folder is sitting where it was, holding both. Anything that can wear an icon can go in: attachments, pictures, videos, models, decks of flip cards and code cells. Country cards are the one exception, and only half of one: they go into a folder that already exists, but two of them dropped on each other click together instead of filing (see **Countries that click together**). Drop a third thing onto the folder and it joins them. Or make an empty one first with **Folder** in the add menu and fill it later.
...
**A card is a little board, not a form.** Everything on it — the question, a picture, a video, a model, the options, a whole widget off the palette — is a block you can pick up, drag anywhere and size to taste. A new card starts the way a study card usually is: **one line, written big, sitting in the middle of the card**, because most of them never hold more than that.

### The card stock

**◑** on the item's toolbar (and on the tray while you write, and in the scope's bar) opens the stock: **eight looks**, each a chip in its own colours. **Index card** is the classic — white stock, the red rule. **Plain** is quiet, no rule, softer corners. **Notebook** is ruled right down the card with the margin line. **Kraft** is brown card, stitched round the edge. **Sticky** is a square of yellow paper in your own hand. **Chalkboard** is slate and chalk in a wooden frame. **Blueprint** is white lines on blue, set in the typewriter face. **Night** is dark glass with a glow at the edge, for a dark room. A look is the whole deck's — the cards, the sheets behind them, the scoreboard — and it goes with the deck into print, into an export and into the file it studies from.
...
- **Right-click the card and the palette opens for it** — the same panel the sheet uses, with its shelves, its icons and its search, saying *onto the card* — and whatever you pick lands on that side, **exactly where you clicked**. A table, axes with curves on them, a pie / bar / stacked chart, a matrix or a vector, a checklist, a code cell, a sticky, a cube, sphere or torus to draw over, a molecule, the periodic table, the chart of nuclides, a Feynman diagram, tape, a sticker: it is the very same item the page would hold, drawn by the same feature, so **it works on the card** — the table's cells take numbers and formulas, the plot pans, the molecule turns — and picked out, **it wears the same toolbar it wears on the canvas**, hanging right over it — the feature's own buttons (the table's rows and columns, the chart's kind and palette, the plot's expressions), A− / A+ where the writing sizes, ⌖ to centre it and ✕ to take it off. **Drag it from anywhere on it**, exactly as on the sheet — a surface its feature claims (a cell, a legend, a plot mid-pan, a model or a solid you turn by hand) keeps the hand, and the grip above it is always there too. Picture, Video, Model and Attachment on the palette open the card's own file dialogs, so the palette is the one place for everything. Anything that needs the sheet around it — a logic circuit, a node graph, a country or the world, a slide deck, a folder, another deck — is dimmed and stays on the sheet; a feature added tomorrow is on the card the day it registers, since the palette is built from the registry. **Widget** on the tray opens the same palette without a spot in mind. Move a widget by the **grip** that appears above it; the dot on its corner sizes it. A widget is measured in the card like everything else on it, so it grows with the card in the scope and prints at the size it was written.
...
**Tap the card and it turns over — anywhere on it, a widget included.** While you study, a widget is part of the card's picture and never swallows the tap; open the writing tray and it wakes up again. Decide how you did and hit **✓** or **✗** — the deck moves on to the next card by itself. The deck's buttons are all drawn with the same pen — small stroke icons, one line each; hover any of them for what it does. A multiple choice card marks itself the moment you pick, showing the right answer in green and your mistake in red, then turns over to the explanation if there is one. The score sits in the corner of the deck (`8✓ 2✗`), with a line under the title bar filling up as you get through the run.

- **⌖ gives the card the whole screen.** The sheet, the desk and everything else fade out behind it and the card comes up big, on its own. Because everything on a card is measured in the card itself, the same card is simply *bigger* in here — nothing reflows, nothing has to be laid out twice. The keys do the work: **space** turns the card, **← →** walk the deck, **1–9** pick an answer, **r** and **w** mark it right or wrong, **s** brings the score up, **esc** puts the sheet back. Here too the whole card takes the tap: a widget, a model, a chart on it is part of the picture while you study.
- **Throw the card.** In the scope, drag it **right for ✓, left for ✗** — and the card tells you which way it is going as you go: the **✓ RIGHT stamp** comes up in its corner and the face washes green as it heads right, the **✗ WRONG stamp** and a red wash as it heads left, and the two gate posts at the edges of the stage (`‹ ✗` and `✓ ›`) brighten on the side it is heading for. The marks under the card sit the same way round, ✗ left and ✓ right, and the line at the foot says so. It follows your hand, and on release the speed of the throw decides, or it springs back to the middle for another look.
- **Σ is the scoreboard.** A ring of how the run stands, three big numbers — right, wrong, to go — and a sentence about it, set to be read from across the room. Under that, **Over time**: every run you have finished, one bar each, the newest on the right, with the average drawn across them, and beside them how many runs, your best, your average, whether the last five are up or down on the five before, and how many clean sheets in a row. A run on the missed cards only is drawn in the second colour. Then **Keeps getting away** — the three cards you have got wrong most often, with their tallies — and **This run**, every card listed with how it went. Click any line to jump straight to that card. It comes up on its own when you finish the last card.
...
A deck keeps a running tally per card, so you can tell an old friend from one you have never once got right — and a record of every finished run, so you can tell whether the deck is sinking in. A mark you change after the last card corrects that run's line rather than adding another; a replay is a fresh sitting and gets a line of its own.

The sounds are the card's: a breath of air as one turns, a shorter one as the next slides in, a longer one as a card is thrown, and two small notes for the marks — a rising fifth for ✓, a falling fourth for ✗ — with a short three-note close when the run is done. They follow the app's sound setting and volume like everything else.

### Taking a deck to the desk

**⤓ on the item's toolbar (or in the scope's bar) offers two ways out of the app.**

**Put it on the desk** opens the deck in **a small window of its own** — not a tab, and not tied to Open Note: close the app and the card stays where it is, still turning, still marking. In the desktop app it is a frameless window that floats above the rest, held by its own bar, closed by the ✕ in it, and **remembered** — quit Open Note and start it again and the cards you had out come back where they were. Every card is in it, both sides, in the deck's look, with its pictures, a video as long as it is not enormous, a model as its still, and the widgets drawn as they print; and the whole of studying: the card turns on a click or **space**, **← →** walk the deck, **r / w** and the two buttons mark it, **1–9** answer a multiple choice card and it grades itself, **throwing the card** works with the same stamps and washes, **s** and Σ bring up the same scoreboard, **↻ / ↻✗** replay all of it or the missed ones. What it learns it keeps, so a desk card carries on where you left off — and it keeps its own record, separate from the deck in the note. Asking again after you have changed the cards rewrites the desk card and the open window reloads to it. In a browser the same thing is a popup window handed the deck directly — no storage in between, so it works however the app is being served, `file://` included — and it lives as long as the browser does.

**Save it as a file** writes the same thing as one `.html` — for a phone, a stick, or anyone who does not have the app. It keeps what it learns in whichever browser it is opened in.
...
In **print** a deck shows the card it is on, question side up, widgets and all. In an **export the cards still turn** — click one and it flips, with the arrows walking the deck, all of it done with no script at all.
