...
inject = ('<img src="/slow?ms=60000" alt="" style="position:fixed;left:-9999px;width:1px;height:1px">\n'
          '<script src="globeshot.js"></script>\n')
...
PROF="$HERE/shotsprof"; rm -rf "$PROF"; mkdir -p "$PROF"
...
OUT="${OUT:-$HERE/shots/png}"
echo "================ SHOTS ================"
python3 "$HERE/globeshot.py" unpack "$RUN/report.txt" "$OUT"
