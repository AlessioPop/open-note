...
const mtrans = m => [m[0], m[2], m[1], m[3]];
const mIdent = m => Math.abs(m[0] - 1) + Math.abs(m[1]) + Math.abs(m[2]) + Math.abs(m[3] - 1) < 1e-9;
const minv = m => { const d = mdet(m); return Math.abs(d) < 1e-12 ? null : [m[3] / d, -m[1] / d, -m[2] / d, m[0] / d]; };
function meig(m){                                  /* λ² − (a+d)λ + det = 0 */
  const tr = m[0] + m[3], disc = tr * tr / 4 - mdet(m);
  if(disc < -1e-12) return { real:false, re:tr / 2, im:Math.sqrt(-disc) };
  const s = Math.sqrt(Math.max(0, disc));
  return { real:true, l1:tr / 2 + s, l2:tr / 2 - s };
}
...
onBookOpen(() => { mathSel = null; PLOT_MOVE.clear(); });
