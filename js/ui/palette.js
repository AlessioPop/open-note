...
function openQuickMenu(x, y, ctx, above, ox, oy){
  qCtx = ctx || null;
  palClosing = false;
  const bp = (ctx && ctx.page) || (index && activePage());
  $('#bmarkBtn .lb').textContent = bp && (index.bookmarks || []).some(b => b.pageId === bp.id)
...
window.addEventListener('pointerdown', e => {
  if(qmenu.classList.contains('open') && !e.target.closest('#palette') && e.target.id !== 'qaddBtn')
    closeQuickMenu();
  if(bmChip && !e.target.closest('.strchip') && !e.target.closest('.bmark')) closeBmChip();
});
