...
/* same-page strings live in page.links = [{id, a, b, c}]; strings across a spread
   live in index.xlinks = [{id, ap, a, bp, b, c}] (page + item on each end).
   One svg board covers the whole open view, so yarn can cross the gutter. */
...
function itemLinked(page, id, bIdx){
  if((page.links || []).some(l => l.t !== 'arr' && (l.a === id || l.b === id))) return true;
  const idx = bIdx || index;
  return !!(idx && idx.xlinks && idx.xlinks.some(x => x.t !== 'arr' && (x.a === id || x.b === id)));
}
...
function drawStaticStrings(wrap, page, bIdx){
  ...
  ((idx && idx.xlinks) || []).forEach(x => {
    if(x.t === 'arr') return;
    if(x.ap === page.id) pinIds.add(x.a);
    if(x.bp === page.id) pinIds.add(x.b);
  });
  pinIds.forEach(id => {
...
/* the live board: one overlay over the whole view (both pages of a spread) */
function buildBoard(entries){
  ...
  for(const en of entries){
    ...
    (en.page.links || []).forEach(l =>
      addRope({ kind: 'page', page: en.page, link: l, ap: en.page.id, a: l.a, bp: en.page.id, b: l.b }));
  }
  ((index && index.xlinks) || []).forEach(x => {
    if(wraps[x.ap] && wraps[x.bp])
      addRope({ kind: 'x', link: x, ap: x.ap, a: x.a, bp: x.bp, b: x.b });
  });
  wakeRopes();
}
function addRope(r){
...
function selectString(rope){
  ...
  const saveLink = () => { if(rope.kind === 'x') queueIndex(); else queueSave(rope.page.id); };
  if(rope.arrow){
    ...
    btn('⇄', 'Flip direction', () => {
      const l = rope.link;
      if(rope.kind === 'x'){ [l.a, l.b] = [l.b, l.a]; [l.ap, l.bp] = [l.bp, l.ap]; }
...
function deleteString(rope){
  if(rope.kind === 'x'){
    index.xlinks = (index.xlinks || []).filter(l => l.id !== rope.link.id);
    queueIndex();
  } else {
...
function createLink(page, aId, bId, kind){
  ...
  if(BOARD && BOARD.wraps[page.id]){
    ...
    addRope({ kind: 'page', page, link, ap: page.id, a: aId, bp: page.id, b: bId });
    wakeRopes();
  }
  SND.pop();
}
function createXLink(pa, aId, pb, bId, kind){
  index.xlinks = index.xlinks || [];
  const t = kind === 'arr' ? 'arr' : 'str';
  if(index.xlinks.some(x => linkKindOf(x) === t &&
     ((x.a === aId && x.b === bId) || (x.a === bId && x.b === aId)))) return;
  const link = { id: uid(), ap: pa.id, a: aId, bp: pb.id, b: bId, c: 0 };
  if(t === 'arr'){ link.t = 'arr'; link.st = index.settings.arrowStyle || 'straight'; }
  index.xlinks.push(link);
  queueIndex();
  if(BOARD && BOARD.wraps[pa.id] && BOARD.wraps[pb.id]){
    if(t !== 'arr') [[pa.id, aId], [pb.id, bId]].forEach(([pid, id]) => {
      const el = BOARD.wraps[pid].querySelector('.item[data-id="' + id + '"]');
      if(el) ensurePin(el);
    });
    addRope({ kind: 'x', link, ap: pa.id, a: aId, bp: pb.id, b: bId });
    wakeRopes();
  }
  SND.pop();
}
/* rebuild every live rope from the current link records (after items change page) */
function rebuildRopes(){
  ...
  for(const en of BOARD.entries)
    (en.page.links || []).forEach(l =>
      addRope({ kind: 'page', page: en.page, link: l, ap: en.page.id, a: l.a, bp: en.page.id, b: l.b }));
  ((index && index.xlinks) || []).forEach(x => {
    if(BOARD.wraps[x.ap] && BOARD.wraps[x.bp])
      addRope({ kind: 'x', link: x, ap: x.ap, a: x.a, bp: x.bp, b: x.b });
  });
  wakeRopes();
}
/* move an item to the facing page mid-drag; its strings change kind as needed */
function transferItem(it, el, from, to){
  from.items = from.items.filter(x => x !== it);
  it.z = maxZ(to) + 1;
  to.items.push(it);
  el.style.zIndex = 6 + it.z;
  BOARD.wraps[to.id].querySelector('.surface').appendChild(el);
  /* cross-page links follow the item; ones landing beside their partner turn local again.
     a/b order is preserved throughout — arrows are directional */
  if(index.xlinks && index.xlinks.some(x => x.a === it.id || x.b === it.id)){
    const keep = [];
    for(const x of index.xlinks){
      if(x.a !== it.id && x.b !== it.id){ keep.push(x); continue; }
      const otherPage = x.a === it.id ? x.bp : x.ap;
      if(otherPage === to.id){
        to.links = to.links || [];
        const nl = { id: x.id, a: x.a, b: x.b, c: x.c || 0 };
        if(x.t){ nl.t = x.t; nl.st = x.st; }
        to.links.push(nl);
      } else {
        keep.push(x.a === it.id ? { ...x, ap: to.id } : { ...x, bp: to.id });
      }
    }
    index.xlinks = keep;
    queueIndex();
  }
  /* same-page links whose partner stayed behind become cross-page links */
  const split = (from.links || []).filter(l => l.a === it.id || l.b === it.id);
  if(split.length){
    from.links = from.links.filter(l => !split.includes(l));
    index.xlinks = index.xlinks || [];
    split.forEach(l => {
      const x = { id: l.id, c: l.c || 0,
        ap: l.a === it.id ? to.id : from.id, a: l.a,
        bp: l.b === it.id ? to.id : from.id, b: l.b };
      if(l.t){ x.t = l.t; x.st = l.st; }
      index.xlinks.push(x);
    });
    queueIndex();
  }
  queueSave(from.id); queueSave(to.id);
  activePageId = to.id;
  rebuildRopes();
}
