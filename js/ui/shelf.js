/* Open Note — ui/shelf.js
   the shelf — every sketchbook as a mini cover */
...
async function openShelf(){
  ...
  const sh = $('#shelf');
  sh.classList.toggle('noBook', !curBookId);
...
async function buildShelf(){
  ...
  const books = [...lib.books].sort((a, b) => (b.updated || 0) - (a.updated || 0));
  for(const b of books){
    const doc = await kvGet(kBook(b.id));
    const card = document.createElement('div'); card.className = 'bk' + (doc && isCanvas(doc) ? ' cnv' : '');
    ...
    const csz = pgSize(doc);                       // covers keep their own book's page shape
    card.style.setProperty('--pw', csz.w); card.style.setProperty('--ph', csz.h);
    let pg = null, cover = null;
    if(doc && doc.pages.length){
      cover = await kvGet(kPage(doc.pages[0].id));
      if(cover) pg = buildPage(cover, false, MEDIA_URL, doc);
    }
    if(!pg){ pg = document.createElement('div'); pg.className = 'page'; }
    card.appendChild(pg);
    const npages = doc ? Math.max(0, doc.pages.length - 1) : 0;
    /* a canvas has no pages to count — it says how big the sheet has grown */
    const what = doc && isCanvas(doc) ? '∞ ' + csz.w + ' × ' + csz.h
    ...
    card.insertAdjacentHTML('beforeend',
      ...
      '<div class="bkmeta">' + what + (b.updated ? ' · ' + fmtDate(b.updated) : '') + '</div>' +
      '<button class="del" title="Delete sketchbook">✕</button>');
    ...
    del.addEventListener('click', async e => {
      e.stopPropagation();
      if(!confirm('Delete "' + (b.name || 'this sketchbook') + '" and everything in it? This cannot be undone.')) return;
      await deleteBook(b.id);
      $('#shelf').classList.toggle('noBook', !curBookId);
      buildShelf();
    });
    grid.appendChild(card);
    fit(pg);
    if(cover) drawStaticStrings(pg, cover, doc);
  }
  /* the two things you can start: a book of pages, or one endless sheet */
  mkNew(grid, '+', 'New sketchbook', 'A book of pages', createBook);
  mkNew(grid, '∞', 'New canvas', 'One endless sheet, no pages', createCanvas, 'wide');
}

function mkNew(grid, glyph, label, title, make, cls){
  const nc = document.createElement('div');
  nc.className = 'bk new' + (cls ? ' ' + cls : '');
  nc.title = title;
  nc.innerHTML = '<div class="face"><div><div class="plus">' + glyph +
    '</div><div class="newlab">' + label + '</div></div></div>';
  nc.addEventListener('click', async () => openBook(await make()));
  grid.appendChild(nc);
}
...
async function deleteBook(id){
  ...
  if(doc) for(const m of doc.pages){
    ...
    if(p) for(const it of p.items || []) for(const m of mediaIds(it)){ await mediaDel(m); dropMedia(m); }
  ...
  if(curBookId === id){ curBookId = null; index = null; $('#pageHost').innerHTML = ''; }
...
$('#shelfClose').addEventListener('click', () => { if(curBookId) $('#shelf').classList.remove('open'); });

/* ---- how it looks ---- */
addCSS('shelf', `
/* ---------- library shelf ---------- */
.shelf{position:fixed;inset:0;z-index:55;display:none;overflow:auto;padding:34px 26px 70px;background:#0c0f12 radial-gradient(120% 80% at 50% 0%,rgba(255,255,255,.055),transparent 60%)}
...
.bkgrid{display:grid;grid-template-columns:repeat(auto-fill,minmax(175px,1fr));gap:34px 28px;max-width:1080px;margin:0 auto}
.bk{position:relative;cursor:pointer;transition:transform .22s cubic-bezier(.3,.7,.3,1)}
.bk .page{height:auto;width:100%;max-width:none;pointer-events:none;box-shadow:0 12px 26px rgba(0,0,0,.55);transition:box-shadow .22s}
.bk:hover{transform:translateY(-9px) rotate(-1.3deg)}
...
.bk.new .face{aspect-ratio:1/1.34;border:2px dashed rgba(255,255,255,.25);border-radius:3px;display:grid;place-items:center;color:#a6a39a;background:rgba(255,255,255,.02);transition:border-color .2s,color .2s,background .2s}
.bk.new:hover{transform:translateY(-9px)}
