...
async function dropLayer(L){
  const ls = layers(index);
  if(ls.length < 2){ alert('A sketchbook keeps at least one layer.'); return; }
  const i = ls.indexOf(L), to = ls[i === 0 ? 1 : i - 1];
  if(!confirm('Remove the layer "' + L.name + '"? Everything on it, on every page, moves onto "' + to.name + '".')) return;
  for(let k = 0; k < index.pages.length; k++){
    const p = await loadPage(k);
    let ch = false;
